...
6. 「接続」の項目では、「VPC」でRDSをデプロイしたいVPCを選択する。「既存の VPC セキュリティグループ」でセキュリティグループ名を選択するとそのセキュリティグループからのみ通信を許可するという設定になる。「アベイラビリティーゾーン」ではRDSと接続したいなと思っているインスタンスがあるAZを選択する。
...
2. 次に、正常に接続ができているかの確認を行う。コマンドプロンプトを開く。上記の「sshでEC2に接続」に沿ってEC2に接続する。
